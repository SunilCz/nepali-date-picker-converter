 ...
 */
const yearMonthDaysMapping: number[][] = Object.values(dateConfigMap).map((year) => Object.values(year));
...
/**
 * Max possible Day (computed from the provided dateConfigMap)
 */
const MAX_DAY = daysPassed;
...
export function mapDaysToDate(daysPassed: number): IYearMonthDate {
	...
	const yearIndex = yearDaysMapping.findIndex(
		(year) => daysPassed > year[COMPLETED_DAYS] && daysPassed <= year[COMPLETED_DAYS] + year[TOTAL_DAYS],
	);
	...
	const monthIndex = monthDaysMappings[yearIndex].findIndex(
		(month) =>
			monthRemainder > month[COMPLETED_DAYS] && monthRemainder <= month[COMPLETED_DAYS] + month[TOTAL_DAYS],
	);
...
export function format(bsDate: IYearMonthDate, stringFormat: string, language: "en" | "np"): string {
	return stringFormat
